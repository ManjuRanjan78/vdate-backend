...
let MatchService = class MatchService {
    ...
    normalizePreferenceGender(preferenceGender) {
        if (preferenceGender === undefined ||
        ...
        const normalized = preferenceGender
            .toLowerCase()
            .trim();
        let result;
        if (normalized === 'male') {
    ...
    getAcceptedGenders(gender, preferenceGender) {
        const normalizedGender = gender?.toLowerCase().trim();
        const normalizedPreference = this.normalizePreferenceGender(preferenceGender);
        let targetGenders;
        if (normalizedPreference === 'female') {
            ...
            const defaultTarget = normalizedGender === 'male'
                ? 'female'
                : 'male';
    ...
    isMatchPreferenceCompatible(currentUserGender, currentUserPreference, candidateGender, candidatePreference) {
        ...
        console.log('PREFERENCE_FILTER_START', {
            currentUserGender,
            currentUserPreference,
            candidateGender,
            candidatePreference,
        ...
        const normalizedCandidateGender = candidateGender.toLowerCase().trim();
        const normalizedCurrentGender = currentUserGender.toLowerCase().trim();
        ...
        const currentAccepts = this.getAcceptedGenders(normalizedCurrentGender, currentUserPreference);
        const candidateAccepts = this.getAcceptedGenders(normalizedCandidateGender, candidatePreference);
        const acceptsEachOther = currentAccepts.includes(normalizedCandidateGender) &&
            candidateAccepts.includes(normalizedCurrentGender);
        if (acceptsEachOther) {
        ...
        if (!acceptsEachOther) {
            console.log('PREFERENCE_FILTER_REJECTED', {
                currentUserGender: normalizedCurrentGender,
                currentUserPreference,
                candidateGender: normalizedCandidateGender,
                candidatePreference,
    ...
    getQueuePreferenceKey(userId, field) {
        return `queue_preference_${field}:${userId}`;
    }
    async setQueuePreferences(userId, preferenceGender, preferenceLocation) {
        if (preferenceGender !== undefined &&
            preferenceGender !== null) {
            ...
            await this.redisService.set(this.getQueuePreferenceKey(userId, 'gender'), preferenceGender, 120);
            await this.redisService.set(`user_preference_gender:${userId}`, preferenceGender, 86400);
        }
        if (preferenceLocation !== undefined &&
            preferenceLocation !== null) {
            await this.redisService.set(this.getQueuePreferenceKey(userId, 'location'), preferenceLocation, 120);
        }
    }
    async clearQueuePreferences(userId) {
        await this.redisService.del(this.getQueuePreferenceKey(userId, 'gender'));
        await this.redisService.del(this.getQueuePreferenceKey(userId, 'location'));
    }
    async getQueuePreferenceGender(userId) {
        const queuePref = await this.redisService.get(this.getQueuePreferenceKey(userId, 'gender'));
        if (queuePref !== null) {
            return queuePref;
        }
        return await this.redisService.get(`user_preference_gender:${userId}`);
    }
    async getQueuePreferenceLocation(userId) {
        return await this.redisService.get(this.getQueuePreferenceKey(userId, 'location'));
    }
    async addToQueue(userId, gender, preferenceGender, preferenceLocation) {
        console.log('AUDIT_QUEUE_INSERT_START', {
            userId,
            gender,
            preferenceGender,
            preferenceLocation,
        ...
        console.log('AUDIT_QUEUE_CHECK_DUPLICATES', {
            userId,
            queue,
            usersInQueue: users,
        ...
        if (users.includes(userId)) {
            ...
            await this.setQueuePreferences(userId, preferenceGender, preferenceLocation);
    ...
    async getMatch(currentUserId, gender, preferenceGender, preferenceLocation) {
        ...
        const targetGenders = this.getAcceptedGenders(gender, preferenceGender);
        const oppositeQueues = targetGenders.map((g) => g === 'male'
            ? 'male_waiting'
            : 'female_waiting');
        console.log('QUEUE_SELECTION', {
            currentUserId,
            gender,
            preferenceGender,
            preferenceLocation,
        ...
        for (const queue of oppositeQueues) {
            ...
            console.log('AUDIT_QUEUE_LOOKUP', {
                currentUserId,
                queue,
                usersFound: qUsers,
                count: qUsers.length,
        ...
        if (!users.length) {
            ...
            for (const tGender of targetGenders) {
                ...
                console.log('AUDIT_ONLINE_USERS_LOOKUP', {
                    currentUserId,
                    gender: tGender,
                    usersFound: oUsers,
                    count: oUsers.length,
        ...
        console.log('CANDIDATE_QUERY_RESULT', {
            currentUserId,
        ...
        for (const userId of shuffledUsers) {
            console.log('MATCH_VALIDATION_START', {
                currentUserId,
                candidateId: userId,
            ...
            if (userId === currentUserId) {
                console.log('CANDIDATE_REJECT_REASON', {
            ...
            const hasHistory = await this.redisService.hasMatchHistory(currentUserId, userId);
            if (hasHistory) {
                console.log(`Recently matched skipped: ${userId}`);
                continue;
            }
            const socketId = await this.redisService.get(`socket:${userId}`);
            const heartbeat = await this.redisService.get(`heartbeat:${userId}`);
            ...
            const candidatePreference = await this.getQueuePreferenceGender(userId);
            console.log('AUDIT_PREFERENCE_CHECK', {
                currentUserId,
                candidateId: userId,
            ...
            if (!this.isMatchPreferenceCompatible(gender, preferenceGender, candidateGender, candidatePreference)) {
                console.log('CANDIDATE_REJECT_REASON', {
                    userId: currentUserId,
                    candidateId: userId,
                    gender,
            ...
            if (preferenceLocation?.toLowerCase() ===
                'nearby' &&
                currentUserCity) {
                if (!candidate.city ||
                    candidate.city
            ...
            console.log('MATCH_CANDIDATE_FOUND', {
