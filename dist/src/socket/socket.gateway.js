...
let SocketGateway = class SocketGateway {
    ...
    getSearchIntentKey(userId) {
        return `searching:${userId}`;
    }
    async setSearchIntent(userId, searching) {
        const key = this.getSearchIntentKey(userId);
    ...
    async isSearchIntentActive(userId) {
        const value = await this.redisService.get(this.getSearchIntentKey(userId));
        return !!value;
    }
    async getQueueState(userId) {
        const active = await this.redisService.get(`queue:${userId}`);
        return active ? 'waiting' : 'idle';
    }
    async getSocketForUser(userId) {
    ...
    async logMatchmakingState(event, userId, data = {}) {
        const queueState = await this.getQueueState(userId);
        const isSearching = await this.isSearchIntentActive(userId);
        const isInCall = !!(await this.redisService.isBusy(userId)) ||
            !!(await this.redisService.getMatched(userId));
        console.log(event, {
            userId,
            queueState,
            isSearching,
    ...
    async resumeUserSearch(userId) {
        const intent = await this.isSearchIntentActive(userId);
        if (!intent) {
            return;
        }
        const socket = await this.getSocketForUser(userId);
        if (!socket) {
            return;
        }
        ...
        const preferenceGender = await this.matchService.getQueuePreferenceGender(userId);
        const preferenceLocation = (await this.matchService.getQueuePreferenceLocation(userId)) || '';
        await this.logMatchmakingState('SEARCH_RESUMED', userId, {
            preferenceGender,
            preferenceLocation,
        });
    ...
    async handleConnection(client) {
        try {
            ...
            await this.redisService.set(`heartbeat:${userId}`, String(Date.now()), 60);
        ...
        try {
            ...
            for (const [channel, timer] of this.activeCallTimers.entries()) {
                if (channel.includes(`_${userId}_`) || channel.endsWith(`_${userId}`)) {
                    ...
                    await this.resumeUserSearch(otherUserId);
                }
            }
    ...
    async findMatch(client, data) {
        ...
        try {
            ...
            console.log('MATCH_START_REQUEST', {
                userId,
            ...
            console.log('MATCH_REQUEST_RECEIVED', {
                currentUserId: userId,
                currentGender: gender,
                currentPreference: data?.preferenceGender || '',
                regionMode: data?.preferenceLocation || '',
            });
            ...
            await this.setSearchIntent(userId, true);
            ...
            console.log('AUDIT_FIND_MATCH_CALLED', {
                userId,
                gender,
                preferenceGender,
                preferenceLocation,
            ...
            console.log('AUDIT_FIND_MATCH_RESULT', {
                userId,
                matchedUserId,
                found: !!matchedUserId,
            });
            if (!matchedUserId ||
                ...
                console.log('AUDIT_NO_MATCH_ADDING_TO_QUEUE', {
                    userId,
                    gender,
                    preferenceGender,
                    preferenceLocation,
                });
                await this.matchService.addToQueue(userId, gender, preferenceGender, preferenceLocation);
                console.log('QUEUE_JOIN_SUCCESS', { userId, gender });
                ...
                console.log('QUEUE_STATE', { userId, gender, queueLength: queueState.length, queueUsers: queueState });
                await this.logMatchmakingState('QUEUE_JOINED', userId, {
                    preferenceGender,
                    preferenceLocation,
                });
                client.emit('waiting', {
            ...
            if (!this.matchService.isMatchPreferenceCompatible(gender, preferenceGender, matchedUser.gender, await this.matchService.getQueuePreferenceGender(matchedUserId))) {
                console.log('MATCH_VALIDATION_RESULT', {
                    userId: userId,
            ...
            await this.redisService.addMatchHistory(userId, matchedUserId);
            ...
            console.log('ROOM_CREATION_START', {
                userId,
                matchedUserId,
                roomName,
            });
            ...
            console.log('ROOM_CREATION_SUCCESS', {
                userId,
                matchedUserId,
                roomName,
            });
            ...
            await Promise.all([
                new Promise((resolve) => {
                    ...
                    this.server.to(`user_${userId}`).emit('match_found', callerPayload);
                    resolve(true);
                }),
                new Promise((resolve) => {
                    console.log('ROOM_EMIT_MATCH', {
                    ...
                    this.server.to(`user_${matchedUserId}`).emit('match_found', receiverPayload);
                    resolve(true);
                }),
            ]);
            console.log('AUDIT_ROOM_CREATED', {
                currentUserId: userId,
                candidateUserId: matchedUserId,
                roomName,
            });
            console.log('AUDIT_MATCH_CREATED', {
                currentUserId: userId,
                candidateUserId: matchedUserId,
                callId,
            });
            console.log('AUDIT_CALL_STARTED', {
                currentUserId: userId,
                candidateUserId: matchedUserId,
                roomName,
                callId,
            });
            await this.logMatchmakingState('CALL_STARTED', userId, {
                matchedUserId,
                roomName,
                callId,
            });
    ...
    async startDirectCall(client, data) {
        ...
        try {
            ...
            if (matchedUser.fcmToken) {
                await this.firebaseService.sendNotification(matchedUser.fcmToken, 'Incoming Video Call', `${currentUser.name || 'Someone'} is calling you`, {
                    type: 'incoming_call',
            ...
            setTimeout(async () => {
                try {
                    ...
                    if (callData && callData.status === 'started') {
                        ...
                        if (matchedUser.fcmToken) {
                            await this.firebaseService.sendNotification(matchedUser.fcmToken, 'Missed Video Call', `You missed a call from ${currentUser.name || 'Someone'}`, {
                                type: 'missed_call',
    ...
    async handleCallConnected(data) {
        try {
            ...
            console.log('CALL_CONNECTED', { roomName, userId, matchedUserId });
    ...
    async leaveCall(data) {
        try {
            ...
            if (!resumeIntent) {
                await this.setSearchIntent(userId, false);
            }
            await this.redisService.removeBusy(userId);
            await this.redisService.removeBusy(matchedUserId);
            await this.redisService.clearMatched(userId);
            ...
            await this.redisService.del(`queue:${userId}`);
            await this.redisService.del(`queue:${matchedUserId}`);
            await this.logMatchmakingState('CALL_ENDED', userId, {
                matchedUserId,
            ...
            if (matchedUserId) {
                await this.resumeUserSearch(matchedUserId);
            }
            this.server
            ...
            if (!callWasConnected) {
                try {
                    ...
                    if (matchedUser && matchedUser.fcmToken) {
                        await this.firebaseService.sendNotification(matchedUser.fcmToken, 'Missed Video Call', `You missed a call from ${caller?.name || 'Someone'}`, {
                            type: 'missed_call',
                            callerId: String(userId),
    ...
    async endCallDueToCoins(channelName, userId1, userId2) {
        ...
        await this.logMatchmakingState('CALL_ENDED', userId1, {
            matchedUserId: userId2,
            reason: 'Insufficient coins',
        });
        await this.logMatchmakingState('CALL_ENDED', userId2, {
            matchedUserId: userId1,
            reason: 'Insufficient coins',
        });
        await this.resumeUserSearch(userId1);
        await this.resumeUserSearch(userId2);
    }
    async handleLeaveQueue(data) {
        try {
            ...
            await this.logMatchmakingState('QUEUE_REMOVED', userId);
        }
        catch (error) {
...
__decorate([
    (0, websockets_1.SubscribeMessage)('reject_call'),
    __param(0, (0, websockets_1.ConnectedSocket)()),
    __param(1, (0, websockets_1.MessageBody)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [socket_io_1.Socket, Object]),
    __metadata("design:returntype", Promise)
], SocketGateway.prototype, "handleRejectCall", null);
__decorate([
